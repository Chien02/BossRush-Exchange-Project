...
@export var distance_in : float = 150
@export var dead_zone : float = 100
...
func zoom_focus():
	if (player.target):
		...
		if (target_position.length() < dead_zone):
			tween.parallel().tween_property(main_camera, "zoom", Vector2(3.2, 3.2), 0.5)
			...
			tween.parallel().tween_property(main_camera, "zoom", Vector2(3, 3), 0.5)
